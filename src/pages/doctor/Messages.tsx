...
import { Search, Send, Paperclip, Loader } from 'lucide-react';
...
// Helper function to handle role type incompatibility
...
export const DoctorMessages = () => {
  ...
  const sentMessagesRef = useRef<Set<string>>(new Set()); // Add a ref to track sent messages
  ...
  useEffect(() => {
    ...
    console.log("Doctor attempting to connect to socket at:", backendUrl);
    ...
    socketRef.current.on('connect', () => {
      ...
      console.log('Doctor user ID:', currentUser._id);
      fetchConversations();
    });
    ...
    socketRef.current.on('new_message', (newMessage) => {
      console.log('Doctor new message received:', newMessage);

      // Skip messages we've already added optimistically
      if (newMessage.senderId === currentUser?._id) {
        // If we have the temporary ID stored, we can check and avoid duplication
        const tempIdString = newMessage._id.toString();

        // If we've seen this message ID before, don't add it again
    ...
    socketRef.current.on('new_message_notification', ({ message, conversation }) => {
      console.log('Doctor new message notification:', message, 'for conversation:', conversation);
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    const receiver = getPatient(selectedConversation);
  ...
  // Get the patient user from conversation
  const getPatient = (conversation: Conversation): User | undefined => {
    return conversation.participants.find(user => user.role !== 'doctor' && user._id !== currentUser?._id);
  };

  const filteredConversations = conversations.filter(convo => {
    const patient = getPatient(convo);
            ...
            <div className="overflow-y-auto h-[calc(100%-5rem)]">
              {loading && conversations.length === 0 ? (
                ...
                <div className="p-4 text-center text-gray-500">
                  {searchTerm ? 'No patients match your search' : 'No conversations yet'}
                </div>
              ) : (
                filteredConversations.map(conversation => {
                  const patient = getPatient(conversation);
                  if (!patient) return null;

                  return (
                    <button
                      key={conversation._id}
                      className={`w-full p-4 flex items-center hover:bg-gray-50 transition relative ${selectedConversation?._id === conversation._id ? 'bg-primary-50' : ''
                        }`}
                      onClick={() => setSelectedConversation(conversation)}
                    >
                      <Avatar
                        user={formatUser(patient)}
                        size="md"
                        className="mr-3"
                      />
              ...
              <>
                <div className="p-4 border-b border-gray-100">
                  <div className="flex items-center">
                    <Avatar
                      user={formatUser(getPatient(selectedConversation) || {
                        _id: '',
                    ...
                    <div>
                      <h3 className="font-medium text-gray-800">
                        {getPatient(selectedConversation)?.name || 'Unknown Patient'}
                      </h3>
                      <p className="text-sm text-gray-500">
                        {getPatient(selectedConversation)?.medicalHistory ?
                          `${getPatient(selectedConversation)?.medicalHistory?.substring(0, 50)}...` :
                          'No medical history'}
                      </p>
                    </div>
                  </div>
