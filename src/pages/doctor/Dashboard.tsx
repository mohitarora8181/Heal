...
export const DoctorDashboard = () => {
    ...
    const appointmentPatients = todaysAppointments.map(appointment => {
        const patient = mockPatients.find(pat => pat.id === appointment.patientId);
        return {
            appointment,
            patient
        };
                ...
                <motion.div variants={itemVariants}>
                    <Card className="h-full">
                        <CardContent className="flex flex-col items-center justify-center py-6">
                            ...
                            <h2 className="text-3xl font-bold text-gray-800">{mockPatients.length}</h2>
                            ...
                            {appointmentPatients.length > 0 ? (
                                <div className="space-y-4">
                                    {appointmentPatients.map(({ appointment, patient }) => (
                                        <div key={appointment.id} className="flex items-center p-4 rounded-lg border border-gray-100 hover:border-primary-200 transition-colors">
                                            ...
                                            <div className="text-right">
                                                ...
                                                {appointment.isUrgent && (
                                                    <Badge variant="error" className="mt-2">Urgent</Badge>
                                                )}
                                            </div>
                                        </div>
                                    ))}
                                    ...
                                    <div key={patient.id} className="flex items-center">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-gray-500">
                                                {patient.medicalHistory ? patient.medicalHistory : 'No medical history'}
                                            </p>
                                        </div>
