...
import { Search, FileText, Download, Eye, Upload, X, Trash2 } from 'lucide-react';
...
export const Records = () => {
    ...
    const handleDeleteRecord = async (recordId: string) => {
        ...
        try {
            ...
            const response = await fetch(`${backendUrl}/medical-records/${recordId}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
    ...
    const filteredDoctors = doctors.filter(doctor =>
        ...
        (doctor.specialization && doctor.specialization.toLowerCase().includes(doctorSearchTerm.toLowerCase()))
    );
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const recordData = {
                ...newRecord,
                date: newRecord.date instanceof Date ? newRecord.date.toISOString() : new Date().toISOString(),
                fileUrl: fileUrl
            };
            ...
            {loading ? (
                ...
                <div className="flex flex-col items-center justify-center h-64 text-center">
                    ...
                    <p className="text-gray-500 mt-2">
                        {searchTerm ? `No results for "${searchTerm}"` : 'Upload your first medical record to get started'}
                    </p>
                </div>
            ) : (
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <Input
                                            ...
                                            name="date"
                                            type="date"
                                            value={newRecord.date instanceof Date ? format(newRecord.date, 'yyyy-MM-dd') : ''}
                                            onChange={(e) => {
